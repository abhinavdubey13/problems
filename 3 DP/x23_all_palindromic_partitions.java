 ...
 * approach : 
 * ============
 * we can check the general approach from here :
 * 
 * https://leetcode.com/problems/palindrome-partitioning/discuss/182307/Java%3A-Backtracking-Template-General-Approach
 * 
 * 
 * 
 * for optimization , we can create a 2D DP array , so that we can make a look-up in O(1) time , if the string is palindrome 
 * 
 *
 * 
 * ==============
...
class x23_all_palindromic_partitions {
    ...
    static void dfs(String input_string, int start_idx, List<List<String>> answer, List<String> possible_partition) {
        ...
        for (int end_idx = start_idx; end_idx < n; end_idx++) {
            ...
            if (is_palindrome) {
                possible_partition.add(partition);
                dfs(input_string, end_idx + 1, answer, possible_partition);
                possible_partition.remove(possible_partition.size() - 1);
            }
        }
